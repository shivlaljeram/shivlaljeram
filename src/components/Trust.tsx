...
export default function Trust() {
  ...
  return (
    <section id="about" className="py-16 sm:py-24 bg-bg-primary border-t border-border">
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8" ref={sectionRef}>
        <div className="text-center max-w-xl mx-auto mb-12">
          <span className="inline-flex items-center gap-1.5 rounded-lg border border-gold/20 bg-gold-subtle px-3 py-1 mb-4">
            ...
            <span className="text-[10px] font-semibold text-gold-light uppercase tracking-wider">Why JSD</span>
          </span>
          <h2 className="text-2xl sm:text-3xl font-bold text-text-white font-serif">Trusted for Generations</h2>
          <p className="mt-2 text-sm text-text-muted">Over 75 years of excellence in crafting premium mukhwas</p>
        </div>

        <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 sm:gap-4">
          {features.map((f, i) => (
            <div key={i} className={`animate-on-scroll rounded-xl border p-5 sm:p-6 text-center ${i === 0 ? 'border-gold/20 bg-gradient-to-br from-gold-subtle to-transparent' : 'border-border bg-bg-card backdrop-blur-sm hover:border-accent/20'} transition-all duration-300 hover:-translate-y-0.5 hover:shadow-lg`} style={{ transitionDelay: `${i * 100}ms` }}>
              <div className={`flex items-center justify-center h-12 w-12 rounded-xl mx-auto mb-3 ${i === 0 ? 'bg-gold/10 text-gold-light' : 'bg-accent-subtle text-accent-light'}`}>
                {icons[i]}
              </div>
